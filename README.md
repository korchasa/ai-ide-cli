...
All runtimes accept optional `env`, `onEvent` (raw NDJSON escape hatch),
`hooks` (typed lifecycle), `onToolUseObserved` (Claude/Codex), and
`settingSources` (Claude cleanroom config):
...
OpenCode and Codex run a stdio MCP server for Human-in-the-Loop. The
library does NOT ship a binary — it exposes the MCP handlers
(`runOpenCodeHitlMcpServer`, `runCodexHitlMcpServer`) and requires the
consumer to supply a zero-argument callback returning the `argv` that
spawns that handler in a sub-process.
